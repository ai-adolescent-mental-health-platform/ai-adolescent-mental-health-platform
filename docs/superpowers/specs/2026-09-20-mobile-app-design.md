...
| 心理咨询 | 咨询师目录与详情、预约下单、与咨询师图文咨询、咨询师入驻申请 |
...
| 真实支付网关 | 见第 7 节。后端无任何支付对接，本版沿用后台人工标记 |
...
### 2.3 `packages/api-client` 依赖 axios 与 fetch，小程序端不可用
...
**微信小程序运行时既没有 `XMLHttpRequest` 也没有 `fetch`**，uni-app 编译到小程序端时二者均不可用。因此该包不能原样复用，这是第 4 节存在的原因。
...
这是一处**已知并接受的取舍**，不是疏忽：手机端持有的是未成年人心理健康数据对应的会话凭据，设备丢失或应用沙箱被突破的后果比浏览器更重。但加密存储并非跨端可用（`plus.storage` 一类只有 App 端有，小程序没有），首版强行引入会同时增加两端实现复杂度与不稳定性。
...
web-client 的流式实现是裸 `fetch` 读流（`packages/api-client/src/index.ts:780`）。

**各端能力不一致：**

| 端 | 流式能力 |
| --- | --- |
| H5 | 有原生 `fetch`，与 web-client 同路径 |
| 微信小程序 | 无 `fetch`，只能用 `uni.request({ enableChunked: true })` + `onChunkReceived` 接收分块，且需要基础库支持 |
| App（原生打包） | 另一套实现，行为与小程序不同 |

**因此不能假设「一套代码三端都能流式」**，这是必须在目标端实测的事，不是配置项。

**处置：**

- **首选**：在目标端实现分块接收并自行解析 SSE 帧。保留流式体验，代价是逐端验证。
...
四步表单流程，对应 `components/apply/` 下的 `intro-page` / `basic-page` / `form-page` / `status-page`。量大但机械。

**待确认**：这是给「想成为咨询师的人」用的流程，放在面向青少年的手机端 App 里是否合适。若能确认目标人群不重合，可考虑本版不做。
...
**结论：现有的「支付」是后台人工标记的，不是真实在线支付。**

**本版决定：沿用现状。** 手机端完成下单与订单查看，支付状态仍由后台人工标记，与 web-client 行为一致。

**这是一处需要明示给产品/负责人的体验断裂**：App 与小程序用户的心理预期是在应用内付款，找不到付款入口会困惑。本期接受该断裂，但**必须在应用内给出清晰的下一步说明**（例如告知用户线下支付方式或联系客服），不能只留一个「未支付」的状态标签。

若后续要做真实支付，那是**独立的后端工程**：商户号申请、后端统一下单接口、支付回调验签、退款流程，加上 uni-app 的 `uni.requestPayment` 前端接入。工作量可能超过手机端其余所有模块之和，**建议单独立项，不要塞进本模块的分期**。
...
| 第 3 期 | 菜单 2：咨询师目录/详情 → 图文咨询 → 入驻申请 → 预约下单 | 图文咨询涉及第二条实时通道，独立验证 |
...
| 目标交付端：只做 App，还是同时要小程序 | 负责人 | **影响流式方案能否落地**（见 6.1）与录音、推送等权限申请 |
| 咨询师入驻申请是否放在手机端 | 负责人 | 见 7.2，影响菜单 2 范围 |
| `packages/config` 的具体内容与可用性 | 实施同学核实 | 影响是否复用 |
| 未支付状态下的引导文案：告知线下支付方式，还是引导联系客服 | 负责人 | 见 7.4，影响预约下单的收尾体验，决定用户是否会卡在「未支付」 |
...
3. **支付体验断裂。** 见 7.4。App 用户找不到付款入口，这是本期已知并接受的缺口。
