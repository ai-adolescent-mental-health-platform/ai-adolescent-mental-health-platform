> **已过时（2026-09-15）**：本文描述的是上一代模型 `qwen3.5-omni-plus-realtime`。
> 当前线上模型为 `qwen-audio-3.0-realtime-plus`，其音色体系、`session.update` 字段
> 与默认值均有差异（详见 [小爱倾听Realtime接入现状.md](小爱倾听Realtime接入现状.md)）。
> 本文保留仅作历史参考，迁移时请勿照此修改。
