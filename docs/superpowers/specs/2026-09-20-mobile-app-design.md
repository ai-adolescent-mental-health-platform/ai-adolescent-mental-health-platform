...
**本版实现三块，按风险从低到高排列，建议按此顺序实施。** 7.1（目录与详情）纯读、风险最低；7.3（图文咨询）涉及第二条实时通道，需单独验证；7.4（预约下单）依赖后端虚拟支付的改造。

第 7.2 节的入驻申请**本版不做**，保留标题仅为记录，不参与排序。
...
**基调：以绿色方向为主。** 具体的配色、圆角、字体与动效细节由负责人最终敲定，本文档只锁定方向与硬约束，**不预设具体色值**——色值一旦写进计划就会变成返工点。
